-- sexp_core.lua : S-expression evaluator used by fcitx5-sexp-calc
-- SPDX-License-Identifier: MIT
-- Copyright (c) 2026 Yusuke Furukawa
--
-- Plain Lua 5.3+ with no fcitx5 dependency, so it can be reused by other
-- front ends (or tested with a standalone interpreter). Returns a table:
--   tokenize(src)        -> list of tokens
--   parse(tokens)        -> AST (number, symbol string or list table)
--   eval(ast, env)       -> value
--   evaluate_string(src) -> value
--   format_value(value)  -> string as it should be typed
--   paren_depth(src)     -> 0 when balanced, >0 when open, -1 when over-closed
--
-- Supported:
...
---------------------------------------------------------------------------
-- Parenthesis balance (used by the front end to decide when to evaluate)
---------------------------------------------------------------------------
